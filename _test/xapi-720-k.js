...
(function () {
  ...
  window.__reset = function () {
    ...
    try {
      ...
      ['ratio01_c01_scaled', 'ratio01_c02_scaled', 'ratio01_c03_scaled',
  ...
  /* Seed a state document, for reproducing a resume without walking the unit first.
  ...
  /* סופר completed כפולים — התכונה שהיומן קיים כדי להבטיח.
     ...
     ⚠️ scope is part of the key. Without it the terminal component's two 'onlinelesson'
     statements — its own and the UNIT's — collided: both carry part '...-06' and neither
     carries an objectId, because the unit one is addressed by opts.scope and the real
     library resolves its object id from UNIT_METADATA rather than METADATA. __dupes()
     therefore reported a duplicate on every correct full run, which is worse than useless
     in a helper whose whole job is to be believed. */
  window.__dupes = function () {
    var seen = {}, dupes = [];
    readLog().forEach(function (s) {
      if (s.verb !== 'completed') return;
      var scope = (s.opts && s.opts.scope) || '';
      var k = s.part + '|' + s.objectType + '|' + (s.objectId || '') + '|' + scope;
