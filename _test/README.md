...
| **Pending answers survive leaving** | Every multi-select, single-choice, value-input/dropdown and two-counter screen of every component (23), through the real handlers: pick **without** pressing צדקתי?, then (1) back one screen and forward again, and (2) a reload — capture, a fresh page, `applyExecutionState`. After each, memory, what the screen shows and the button must be exactly what the live handlers left. Added 2026-09-23 for the QA reports on screens 2/3/12/15 (picks in memory, none shown) and 14/16 (fields filled, button dead); on the code before the fix it failed 46 times, and the reload path on **every** such screen of the unit, not only those six. Mutation-checked per painter: each reverted change fails exactly its own screen type. |
| **A first wrong attempt looks the same after leaving** | Every multi-select and single-choice screen (13): one right and one wrong pick (single choice: one wrong pick; screen 2, where every option is correct, an incomplete pick), submit once, then compare every option's classes, `aria-checked` and enabled state, the popup and the button — after back-and-forward and after a reload — against what the live engine drew. Added 2026-09-23: `paintMCQ` redrew a wrong pick as `selected` **and** `wrong` where live shows `wrong` only (14 failures before the fix, on the 7 multi-selects that have a wrong option). |
