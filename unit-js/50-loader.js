'use strict';
/* ═══════════════════ xAPI — loader / init ═══════════════════
   Shared by all six components. Definition-only; 90-boot.js calls bootXAPI() LAST, after every
   per-part hook exists. (Until 2026-09-16 this step could also window.location.replace() to another
   component — the resume hop. The platform owns routing now; see README.md.)
   ...
   Per-part seams:
     ...
     onXapiReady()        optional — runs after the component 'initialized' and the landing
                          screen's item init. Only component 01, the entry component, uses it to
                          open the unit metadata; the other five do not define it. */
...
function bootXAPI() {
  ...
  if (typeof XAPI_METADATA_FILE === 'undefined' || !XAPI_METADATA_FILE) {
    console.warn('[xAPI] XAPI_METADATA_FILE is not defined in this component — reporting is off. ' +
      'See Documentation/reporting-and-resume/ADDING-REPORTING-AND-RESUME.md §2.');
          ...
          try {
            ...
            if (RESUME_ENABLED) {
              try {
                ...
                /* ...and in the harness the saved payload must not pull the frame
                   off the screen the developer just asked for, either */
                if (_payload && !_devHarness) {
