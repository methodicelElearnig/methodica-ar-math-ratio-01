...
/* ── Viewport scaling ──
   ...
   inert in the others (an unused custom property costs nothing), so one version serves all parts
   rather than leaving 01 with a private copy that has to be kept in sync by hand. */
function scaleApp() {
  /* ratio-01's design grid is 1280×710 (not 720) — the only unit-specific
     number in this file. Width fluid too, per the unit's approved scaleApp:
     chrome anchored to canvas edges must reach the real screen edges. */
