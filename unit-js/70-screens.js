'use strict';
/* ═══════════ unit-js/70-screens.js — the screen logic of all six components ═══════════
   Every screen of methodica-ar-math-ratio-01 (0-45), its question engines, painters and the
   component half of resume. Until 2026-09-23 this was six byte-identical copies at the
   foot of each methodica-ar-math-ratio-01-0N/script.js, held together only by the test
   suite's "twins" assertion; it is one file now, as in methodica-math-ratio-02.

   Each component still runs only its own screens: its script.js supplies the
   configuration this file reads at call time (PART_FIRST/PART_LAST, PART_NEXT/PART_PREV,
   the XAPI_* ids and maps, SCREEN_TO_SUBCONTENT, partResult ...), and its index.html
   holds only its own screens' markup.

   LOAD ORDER: after script.js, before 90-boot.js — the position this code always ran
   in. Strict mode, as script.js is: the resume apply assigns through eval. */

/* ═══════════════ resume — the payload contract ═══════════════
   These three lists plus capturePartPayload / applyResumeVars / applyResumeDom /
   restoreScreenUI at the foot of this file (70-screens.js) are the whole per-component half of
